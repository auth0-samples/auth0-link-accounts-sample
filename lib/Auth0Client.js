...
class Auth0Client {
    ...
    return new Promise((resolve, reject) => {
      ...
      request(reqOpts, (error, response, body) => {
        if (error) {
          ...
          return reject(
            `Error getting user with id ${userId}. Status code: ${response.statusCode}. Body: ${body}`
          );
        } else {
  ...
  getUsersWithSameVerifiedEmail(userId) {
    console.log("getUsersWithSameVerifiedEmail - user", userId);
    return new Promise((resolve, reject) => {
      this.getUser(userId)
        .then(user => {
          ...
          request(reqOpts, (error, response, body) => {
            console.log(
              "getUsersWithSameVerifiedEmail err, status, body",
              error,
              response.statusCode,
              body
            );
            if (error) {
              ...
              return reject(
                `Error getting users with same email. Status code: ${response.statusCode}. Body: ${body}`
              );
            } else {
              resolve(JSON.parse(body));
            }
    ...
    return new Promise((resolve, reject) => {
      var reqOpts = {
        ...
        url: `https://${process.env
          .AUTH0_DOMAIN}/api/v2/users/${rootUserId}/identities`,
        headers: {
          Authorization: `Bearer ${process.env.AUTH0_APIV2_TOKEN}`
        },
        json: {
      ...
      request(reqOpts, (error, response, body) => {
        if (error) {
          ...
          return reject(
            `Error linking accounts. Status code: ${response.statusCode}. Body: ${JSON.stringify(
              body
            )}`
          );
        } else {
  ...
  unlinkAccounts(rootUserId, targetUserProvider, targetUserId) {
    return new Promise((resolve, reject) => {
      var reqOpts = {
        method: "DELETE",
        url: `https://${process.env
          .AUTH0_DOMAIN}/api/v2/users/${rootUserId}/identities/${targetUserProvider}/${targetUserId}`,
        headers: {
          Authorization: `Bearer ${process.env.AUTH0_APIV2_TOKEN}`
        }
      };
      ...
      request(reqOpts, (error, response, body) => {
        if (error) {
          ...
          return reject(
            `Error unlinking accounts. Status code: ${response.statusCode}. Body: ${JSON.stringify(
              body
            )}`
          );
        } else {
