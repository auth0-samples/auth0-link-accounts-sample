...
router.get("/", ensureLoggedIn, function(req, res) {
  return Auth0Client.getUser(req.user._json.sub).then(user => {
...
router.get("/suggested-users", ensureLoggedIn, (req, res) => {
  ...
  Auth0Client.getUsersWithSameVerifiedEmail(req.user._json.sub)
    ...
    .catch(err => {
      console.log(
        "There was an error retrieving users with the same verified email to suggest linking",
        err
      );
    })
    .then(() => {
